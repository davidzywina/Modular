AttachSpec("EarlierCode/magma/magma.spec");
Attach("Modular.m"); 
    ...
    if g ge 5 then
        ...
        if IsFinite(K) then     
            ...
            for c in IrreducibleComponents(V) do
                ...
                PolF<[y]>:=PolynomialRing(F,g);
                c_:=ChangeRing(c,F);             
                for a_ in Points(c_) do
                    a1:=Eltseq(a_);

                    if &and [Evaluate(f,a1) eq 0 : f in psi] eq false then continue a_; end if;


                    yy:=[a1[i]+y[i]: i in [1..g]];
                    A_:=[Evaluate(f,yy): f in A];
                    B:=[ [MonomialCoefficient(h,y[i]): i in [1..g]] : h in A_];
                    B:=Matrix(B);
...
"g eq 4";
    ...
    if g eq 4 then
        // TODO: HOMOGENIZE HENSELS LEMMA
        // TODO: double check BUILT IN MAGMA??
        ...
        assert Evaluate(f,[a[i]+x[i]: i in [1..g]]) eq fx+P+fa;
        ...
        mon:=Monomials(fx)[1];
        A:=Coefficients(MonomialCoefficient(fx,mon)*D-MonomialCoefficient(D,mon)*fx);
        A:=[PolK!g: g in A];
      
        J:=ideal<PolK|A>;
        I:=ideal<PolK|[PolK!f: f in psi]>;
        J:=IdealQuotient(J,I);  // only consider pts off curve
        I:=ideal<PolK|Coefficients(P)>;
        J:=Radical(IdealQuotient(J,I));  // ensures that P(x,a) is nonzero
        ...
        if J ne PolK then
            ...
            if IsFinite(K) eq false then
                ...
                for c in [c:c in IrreducibleComponents(V)] do
                    ...
                    for a_ in Points(c_) do                    
                        a1:=Eltseq(a_);
                        if Evaluate(f,a1) eq 0 then continue a_; end if;
"a1=",a1; 
Evaluate(f,a1);
P;
Coefficients(P);
[Evaluate(c,a): c in Coefficients(P)];
assert false;
                        yy:=[a1[i]+y[i]: i in [1..g]];
                        A_:=[Evaluate(f,yy): f in A];                                                
                        B:=[ [MonomialCoefficient(h,y[i]): i in [1..g]] : h in A_];
        ...
        A:=[fa] cat Coefficients(P);
        Delta:=R^2-4*Evaluate(h,[a[i]: i in [1..g]])*Q;
        mon:=Monomials(fx)[1];
        A:=A cat Coefficients(MonomialCoefficient(fx,mon)*Delta-MonomialCoefficient(Delta,mon)*fx);
           
        J:=Radical(ideal<PolK|[PolK!g: g in A]>);
        I:=ideal<PolK|[PolK!h]>;  // h(a) ne 0
        J:=IdealQuotient(J,I);   
        I:=ideal<PolK|Coefficients(MonomialCoefficient(fx,mon)*Q-MonomialCoefficient(Q,mon)*fx)>; 
        J:=Radical(IdealQuotient(J,I));  // ensures that Q(x,a) is not a multiple of f(x)
...
function IsBielliptic(X)
    ...
    */
    ...
    // TODO: enough terms for integrality?
