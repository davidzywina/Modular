...
ModularCurveRec := recformat<
    ...
    // The below entries are currently unused and could be pruned.
    // They are here mainly for compatibility with earlier versions.
    C:Crv, 
...
/*                            
 DESCRIPTION OF SOME OF THE ENTRIES:
    ...
    N           : The above integer N.  Usually the level of G.
    ...
    CPname      : If congruence subgroup Gamma has genus at most 24, then CPname is the Cummins-Pauli label of \pm Gamma.
    _______________________________________________________
    ...
    F           : A sequence giving a basis of M_{k,G} as a Q-vector space.  The first dimMk elements will give a basis over KG (equivalently over C).

       Each F[i] is a modular form; it is given as a sequence of q-expansions at the cusps (with respect to the matrices in "cusps").
       Each q-expansion is a power series in the variable qw:=q^(1/w) with coefficients in Q(zetaN), where w is the width of the cusp being considered. 
    ...
    F0          : Same as above, but for the Q-subspace of M_{k,G} given by prescribed vanishing conditions at the cusps (for example, cusp forms).
                  The first #F0/KG_degree terms will be a basis over KG (equivalently over C).
...
intrinsic CreateModularCurveRec(G::GrpMat, H::GrpMat) -> Rec
    ...
    if X`genus le 24 then
        ...
        I:=CumminsPauliData(X`genus, N0, SL2Index(H0), Sort(widths0));
        assert #I ne 0;

        if #I eq 1 then
            X`CPname:=I[1];  //cp_data[I[1]]`name;
        else
            GL2:=GL2Ambient(N0);
            SL2:=SL2Ambient(N0);
            ...
            for k in I do
                cp_rec:=CumminsPauliData(k);
                H1:=sub<SL2|cp_rec`matgens>;
    ...
    // The following two lines can be slow for larger groups.  Speed up possible. TODO?
...
intrinsic FindModularFormsWithVanishingConditions(M::Rec, mult::SeqEnum : lll:=[true,true], saturation:=true) -> Rec
    {
        ...
        (the parameters "lll" and "saturation" give some control on how LLL and Saturation is done; these should probably be left alone!)
        ...
        Output: 
                ...
                they are linearly independent over the complex numbers).
...
intrinsic FindCanonicalModel(M::Rec : lll:=[true,true,true,true,true], prec0:=0, prec_delta:=10) ->  Rec
 {      
    ...
    if g eq 3 and M`CPname notin gonality_equals_2 then
        ...
        if M`KG_degree eq 1 then
            // When over Q we can change our basis of cusp forms so that 
            // the basis is nicer!
            ...
            M`conversion_matrices[4]:=A*M`conversion_matrices[4];  
            // Need to adjust conversion matrices so that we can compute more precision later.
        end if;
...
// The following is commented out code for morphisms that needs to be reworked.
...
intrinsic FindMorphismCanonical(M,M0)-> SeqEnum 
    ...
    D:=LCM(&cat[[Denominator(c): c in Eltseq(r)] :r in Rows(D)])*D;
...
intrinsic FindMorphismOld(M,M0 : homogeneous:=true, prec0:=0, prec_delta:=10, Id:=[*[]*], mon1:=[* *])-> SeqEnum
    {
            ...
            if d*M`model_degree ge deg_h then
                ...
                if #S ge 1 then
                    ...
                    if zeros le upper_bound_on_number_of_poles then
                        ...
                        // We increase precision and try again
                        return FindMorphismOld(M,M0 :homogeneous:=homogeneous, prec0:=prec+prec_delta, prec_delta:=prec_delta, Id:=Id, mon1:=mon1);
                    end if;
...
intrinsic FindMorphism(M,M0 : homogeneous:=true, prec0:=0, prec_delta:=10, Id:=[*[]*], mon1:=[* *])-> SeqEnum
    ...
    R_QQ<t>:=PowerSeriesRing(Rationals());
    I:=ideal<Pol_FF|I_gen>;
    HS_I:=HilbertSeries(I);  // fix; should know for canonical  (depends on q)

    if false and M_has_canonical_model eq false then //TODO
        //For now, we compute the Hilbert series when we do not have a canonical model.
        //TODO: check running time
        I:=ideal<Pol_FF|I_gen>;
        //HS:=HilbertSeries(Submodule(I));

        HS_I:=HilbertSeries(I);
...
/*
    ...
    //prec:= [Minimum([Valuation(f[i]): f in M_F0])+Minimum([AbsolutePrecision(f[i]): f in M_F0]) : i in [1..M`vinf]];

    prec1:=[&+[Valuation(f[i]):f in M_F0] + Minimum([RelativePrecision(f[i]):f in M_F0]) :i in [1..M`vinf]];
    
    q:=1;
    repeat
        q:=NextPrime(q);
    until M`N mod q ne 0;  // TODO: MAKE SURE IT WORKS!
    KN<z>:=CyclotomicField(M`N);
    ON:=RingOfIntegers(KN);
    Q:=Factorization(ideal<ON|[q]>)[1][1];
    FF_Q,iota:=ResidueClassField(Q);


    
    for d in [1..11] do  //TODO: while loop instead

        if d*M`model_degree lt deg_h then //TODO: FIX
            // Fails constraint on degree that we need after
            // considering number of poles with multiplicity.
            continue d;
        end if;

        "d=",d;
        Pol_KN<[x]>:=PolynomialRing(KN,n);
        all_monomials_of_degree_d:=MonomialsOfWeightedDegree(Pol_KN,d);

        A:=[];
        monomials:=[];

        r_d:=Integers()!Coefficient(R_QQ!Evaluate(HS_I,t+O(t^(d+1))),d);
        
t0:=Realtime();        
        for m in all_monomials_of_degree_d do
            monomials:=monomials cat [m];

            v:=&cat[Coefficients(  ChangePrecision(Evaluate(m,[f[i]: f in M_F0]), prec1[i]) ) : i in [1..M`vinf] ];
            v:=[ON!a: a in v];
            A:=A cat [v];
            
            if #A ge r_d then
                B:=ChangeRing(Matrix(A),iota);
                //r:=Rank(Matrix(B));
                //time r_:=Rank(Matrix(A));
                ...
                A:=[A[i]: i in pivots];
                monomials:=[monomials[i]: i in pivots];
                // Choose linearly independent rows

                if r eq r_d then  //TODO: choose q so that this works!
                    break m;
                end if;
            end if;
...
"TIME",Realtime(t0);
        assert #monomials eq r_d;  //TODO: if not, need more precision!
        //Pol_Q<[x]>:=PolynomialRing(KN,n); 
        //monomials:=[Evaluate(m,[x[i]:i in [1..n]]): m in monomials];

        r:=2; // term of h >1 ??? TODO
        B:=[];
        J:=Sort([O[1]:O in M`cusp_orbits]); 
        for j in J do
            beta:=[Evaluate(m,[f[j]: f in M_F0]) : m in monomials];

            // reorder next two steps somewhat??? TODO
            beta:=&cat[ [b*f: b in M`KG_integral_basis_cyclotomic] : f in beta];
        ...
        C:=Matrix(Basis(NullspaceOfTranspose(B)));
        C:=LLL(C : Proof:=false);
        S:=[ [&+[v[M`KG_degree*(i-1)+j] * M`KG_integral_basis[j]*monomials[i]: i in [1..#monomials], j in [1..M`KG_degree]],
                &+[v[M`KG_degree*(i-1)+j + #monomials*M`KG_degree] * M`KG_integral_basis[j]*monomials[i]: i in [1..#monomials], j in [1..M`KG_degree]]] : v in Rows(C)];

                if #S ge 1 then  // S[1][2] eq 0 ok?
                    ...
                    if zeros le upper_bound_on_number_of_poles then
                        // Not enough info to provable find morphism.
                        // We increase precision and try again
                        return FindMorphism(M,M0 :homogeneous:=homogeneous, prec0:=prec+prec_delta, prec_delta:=prec_delta);
                    end if;
    ...
    for r in [2..#M0`F0 div M0`KG_degree] do
        ...
        while not done do
            ...
            if M_has_canonical_model then
                ...
                HS:=0;  "BROKEN";
                dQ:=Integers()!Coefficient(R_QQ!Evaluate(HS,t+O(t^(d+1))),d);
                // dimension of homogeneous relations of degree d 
            end if;
